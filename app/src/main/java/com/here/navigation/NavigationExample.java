...
public class NavigationExample {
    ...
    public NavigationExample(Context context, MapView mapView) {
        ...
        DataHolder.setNavigationPositionMapMarker(navigationArrow);
        DataHolder.setTrackingPositionMapMarker(trackingArrow);
    ...
    private MapMarker createArrow(int resource) {
        ...
        return new MapMarker(DataHolder.getDefaultMapCenter(), mapImage);
    }
...
//                    updateMapView(currentNavigableLocation.originalLocation.coordinates, currentNavigableLocation.originalLocation.bearingInDegrees);
                    updateMapView(currentNavigableLocation.originalLocation.coordinates, 0.0);
                    return;
                }
    ...
    private void updateMapView(GeoCoordinates currentGeoCoordinates,
        ...
        if (!DataHolder.isIsDragged()) {
            MapCamera.OrientationUpdate orientation = new MapCamera.OrientationUpdate();
            orientation.bearing = bearingInDegrees;
            mapView.getCamera().lookAt(currentGeoCoordinates, orientation, DataHolder.DEFAULT_DISTANCE_IN_METERS);
            navigationArrow.setCoordinates(currentGeoCoordinates);
            trackingArrow.setCoordinates(currentGeoCoordinates);
            DataHolder.setCurrentPositionGeoCoordinates(currentGeoCoordinates);
        }
    }

    private TapListener tapToStartPositionUpdate = new TapListener() {
        @Override
        public void onTap(Point2D point2D) {
            locationProvider.start();
            DataHolder.getMainActivity().findViewById(R.id.clear).setVisibility(View.GONE);
            DataHolder.getMapView().getGestures().setTapListener(tapToStopPositionUpdate);
        }
    };

    private TapListener tapToStopPositionUpdate = new TapListener() {
        @Override
        public void onTap(Point2D point2D) {
            locationProvider.stop();
            DataHolder.getMainActivity().switchRoutingControllers(true);
            DataHolder.getMainActivity().findViewById(R.id.clear).setVisibility(View.VISIBLE);
//            DataHolder.getMapView().getGestures().setTapListener(tapToStartPositionUpdate);
        }
    };
    ...
    public void startNavigation(Route route, boolean isSimulated) {
        ...
        DataHolder.getMapView().getGestures().setTapListener(tapToStopPositionUpdate);
        setupSpeedWarnings();
        setupVoiceGuidance(LanguageCode.ZH_TW);
        DataHolder.getMainActivity().switchRoutingControllers(false);
        if (isSimulated) {
        ...
        navigator.setNavigableLocationListener(new NavigableLocationListener() {
            @Override
            public void onNavigableLocationUpdated(NavigableLocation navigableLocation) {
                GeoCoordinates mapMatchedGeoCoordinates = navigableLocation.mapMatchedLocation.coordinates;
                Double mapMatchedBearing = navigableLocation.mapMatchedLocation.bearingInDegrees;
                MapCamera.OrientationUpdate orientationUpdate = new MapCamera.OrientationUpdate(mapMatchedBearing, 60.0);
                mapView.getCamera().lookAt(GeoShift.getShiftedGeoLocation(mapMatchedGeoCoordinates, 100, mapMatchedBearing), orientationUpdate, 500);
                navigationArrow.setCoordinates(mapMatchedGeoCoordinates);
            }
        });
    }
...
//        LanguageCode ttsLanguageCode = getLanguageCodeForDevice(navigator.getSupportedLanguagesForManeuverNotifications());
        ...
        Locale locale = LanguageCodeConverter.getLocale(languageCode);
